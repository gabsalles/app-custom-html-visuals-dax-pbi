...
import { buildValueFormatDax, getFormatString } from '../../valueFormatDax';
...
// reaproveita o motor já validado dos cards categóricos (utils/categoricalDax.ts:
// ADDCOLUMNS(VALUES())+INDEX()/ORDERBY para o Top-N) em vez de reinventar
// SUMMARIZE/RANKX/CONCATENATEX do zero. Ver plano da Fase de barras categóricas.
// ─────────────────────────────────────────────────────────────────────────
...
export const barChartType: ChartTypeDefinition<BarChartConfig> = {
  ...
  generateDax: (config, ctx) => {
    if (config.categorical) return generateCategoricalBarDax(config, ctx);
    ...
    const colSpan = config.colSpan || 1;
    const rowSpan = config.rowSpan || 1;
    const cardBg = config.cardBackgroundColor || ctx.global.cardBackgroundColor || '#ffffff';
  ...
  renderPreview: (config, ctx) => {
    if (config.categorical) return <CategoricalBarPreview config={config} ctx={ctx} />;
    ...
    return (
      ...
      >
        <div style={{ fontSize: config.fontSizeTitle || 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 8, color: ctx.global.textColorTitle }}>
          {config.title}
        </div>
        {config.bars.map((bar, i) => {
          ...
          return (
            <div key={bar.id} style={{ marginBottom: 8 }}>
              ...
              <div style={{ fontSize: config.fontSizeValue || 16, fontWeight: 800, color: ctx.global.textColorValue }}>{displayValue}</div>
            </div>
          );
        })}
      </div>
